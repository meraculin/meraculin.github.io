...
We found out that there is no correlation between the radius of the droplets and the spring constant, therefore we can calculate the force acted upon the droplets using the average value of the spring constant of FUS protein droplets ($k= 0.00412 pN/)